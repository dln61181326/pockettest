<template>
  ...
  <div class="index">
    ...
    <nav class="navbar navbar-inverse navbar-fixed-top title">
      ...
      <div id="title" class="titlename">已拣货</div>
    ...
    <!-- <div class="ButtonGroup">
      ...
      <div id="pickbutton" class="goodsTipsButton clearfix:after" v-if="keyboardshow" @click="gopicking()">拣货</div>
      <div id="nopickbutton" class="goodsTipsButton clearfix:after" v-if="keyboardshow" @click="toShortage()">缺货</div>
      <div class="clearfix"></div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        pickinglist: store.state.pickitemlist,
        unaorder: "unaorder",
        unacheck: "unacheck",
    ...
    methods: {
      ...
      showprint(printstate, autoprint) {
        ...
        if (printstate == 0) { //header
          ...
          let changecode = 3
          let qrcode = 3
          if (this.orderFrom.startsWith("天猫")) {
            qrcode = 1
            changecode = 2
      ...
      toShortage() {
        ...
        for (var i = 0; i < this.pickinglist.length; i++) {
          ...
          if (this.pickinglist[i].itemFlag != undefined && this.pickinglist[i].itemFlag != "" && this.pickinglist[i]
            .itemFlag != "D" && una != 0 &&
            this.pickinglist[i].reason != undefined && this.pickinglist[i].reason != "" && this.pickinglist[i].reason !=
            null) {
            //需改成商品数 - 拣货数
        ...
        //   for(var i =0;i<this.pickinglist.length;i++){//检查数量是否一致
        //     if(this.pickinglist.items[i].addNum == this.pickinglist.items[i].orderQty){

        //     equalQty = false   //有货品拣货数和库存数 相等
        //     break
        // } 
        //   }
        // for(var i =0;i<this.Shortage.items.length;i++){//检查数量够时是否又选了原因

        //   if(this.Shortage.items[i].addNum != 0 &&  this.Shortage.items[i].reason!=undefined){
        //     equalQty=true
        //     break
        //   }
        // }
        ...
        if (this.Shortage.items.length == 0) {
          ...
          layer.open({
            ...
            yes: (index) => {
              ...
              //   if("100020868"==that.pickinglist[goodsindex].itemCode){
              //    this.$set(that.pickinglist[goodsindex],"AfterShortageGoodsCount",  1)
              //   //  this.pickinglist[goodsindex].afterShortUnaCount
              //   }
              // }
              //         layer.msg('已经走过缺货登记逻辑了')
      ...
      gopicking() {
        ...
        if (this.UnavailableState) {
          ...
          for (var a = 0; a < this.pickinglist.length; a++) {
            ...
            if (this.pickinglist[a].AfterShortageGoodsCount != undefined && this.goodsCount[a] != this.pickinglist[a]
              .AfterShortageGoodsCount && this.pickinglist[a].itemFlag != "D") {
              layer.msg("数量有误，请重新检查")
              return
            } else if (this.pickinglist[a].AfterShortageGoodsCount == undefined && this.pickinglist[a].orderQty != this
      ...
      topicking() {
        ...
        if (this.UnavailableState) {
          ...
          for (var i = 0; i < this.goodsCount.length; i++) {
            if (this.pickinglist[i].AfterShortageGoodsCount == undefined && this.pickinglist[i].itemFlag == "D" || this
