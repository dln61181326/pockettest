...
<div id="searchBox">
  ...
  <input class="searchInput"type="text" v-model="dn" @keyup.enter="tosearchdata(dn,false)" v-on:blur="resetbackground()"placeholder="支持输入或扫描订单号">
  <img class="camera"  @click="showcamerascan()" v-if="showCamera" src="../assets/cameraIcon.png" >

  <div class="searchButton" @click="tosearchdata(dn,false)">搜索</div>
  <div class="clearfix"></div>
...
// },
    ...
    methods: {
      ...
      selectreason(i){
       ...
       var r = this.reasonlist
      //  var reason =  this.orderlist[this.reasonselectindex].reason
...
goCancelOrder:function(id,returnReason,packState){
              ...
              // var url ="http://10.82.26.249:8888/onstore/admin/order/toTake/takeConfirm";
                  ...
                  var url =store.state.runurl+"/onstore/admin/order/express/cancelOrder";
...
down (name) {
      ...
      getmoredata () {
        ...
        if(store.state.returnParam==2){
          ...
          this.getpackagedata(this.dn,true);
        }else if(store.state.returnParam==3){
   ...
   var that = this
     ...
     initbackground:function(){
      ...
      tosearchdata(orderno,refresh){
        // var order = "893941579000070045"
        // this.getcustompickdata(order,refresh)
        // var that = this
      if(store.state.returnParam=="1"){this.getcustompickdata(orderno,false)}
      else if(store.state.returnParam=="2"){this.getpackagedata(orderno,false)}
      else if(store.state.returnParam=="3"){this.getcancelorder(orderno,false)}
    },
...
//  "orderNo": "893941579000070045",
        var d = {
                  ...
                  var url = store.state.runurl+"/onstore/admin/kms/return/readOrder";
                // var url = "/kms"
                  var t = store.state.token
                  $.ajax({
              type: 'post',
            ...
            headers:{
              ...
              this.orderlist.push(d)
               ...
               console.log("f================"+f)
                ...
                else if(this.orderlist[0].channelCode =="OTHER"){
                  this.orderlist[i].channelCodeCN  = "其他"
                 }else{
                  this.orderlist[i].channelCodeCN = "未知渠道"
        ...
        var d = {
                  ...
                  var url = store.state.runurl+"/onstore/admin/order/express/list?pageNum="+this.pagenum+"&flag=1&pageSize=10&orderNo="+orderno;
                // var url = "/kms"
                  var t = store.state.token
                  $.ajax({
              type: 'get',
        ...
        var d = {
            ...
            console.log('store.state.url========'+store.state.url)
                  var url = store.state.runurl+"/onstore/admin/order/express/cancelList?pageNum="+this.pagenum+"&flag=1&pageSize=10&psState=4&orderNo="+orderno;
        ...
        var d = {
                  ...
                  var url = store.state.runurl+'/onstore/admin/kms/dict/getValue';
                  var t = store.state.token
                  $.ajax({
              type: 'post',
            url:url,
...
store.state.cancelreasonArr =   this.reasonlist
      ...
      toDetail:function(i){  
        ...
        // }
        if(store.state.returnParam != 3){
          store.state.deliveryGoodsCode = this.dn 
          store.state.orderlist =  this.orderlist[i]
          store.state.orderDate =  this.orderlist[i].orderDate
          store.state.channelCode =  this.orderlist[i].channelCodeCN
          store.state.orderNo  =  this.orderlist[i].orderNo
        store.state.orderItem =  this.orderlist[i].orderLtem
        store.state.orderAmt = this.orderlist[i].orderAmt
        
        Router.push("CustomerReturnGoodsDetail")
        }
