<template>
  <div class="index">
    ...
    <nav class="navbar navbar-inverse navbar-fixed-top top">
      ...
      <div id="searchBox">
        ...
        <input class="searchInput" type="text" v-model="dn" @keyup.enter="searchpickdata(dn,false)"
          v-on:blur="resetbackground()" placeholder="支持输入或扫描提货码">
    ...
    <div id="third">
      ...
      <div v-if="datasiNull" class="datatips">如输入验证码无效，请检查订单状态是否已取消，无法核查的情况下，请咨询平台电商客服</div>
      ...
      <div class="orderlist">
        ...
        <div class="orderlistContent" v-for="(dataList,index) in orderlist" @click="toDetail(index)">
          ...
          <div class="list1">
            <div class="channelTextDiv ordernum clearfix:after">
              <div class="channelText">        {{dataList.channelCodeCN==undefined?"未知渠道":dataList.channelCodeCN}}
              </div>
            </div>
        ...
        <div class="getmoredata" v-if="false" @click="getmoredata()">加载更多</div>
    ...
    <div class="searchMiddleBox">

      <div id='searchButton' class="searchButton" @click="getpickdata(dn,false)">搜索</div>
    </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        changnnellist: ["", "WS", "WS_MT", "WS_ELM", "TM", "JD", "JDDJ_MZ", "JDDJ_YD", "VM", "7LK", "JK", "BASTORE", "PZ", "MYSTORE",'TM_SM'],
        changnnellistCN: ["全部", "莴笋自营", "莴笋美团", "莴笋饿了么", "天猫商城", "京东商城", "京东到家美妆", "京东到家药店", "自动售货机", "七乐康平台", "平安好医生", "云店", "积分商城", "云店2.0","天猫超市"]
      }
    },
    //     destroyed(){
    ...
    this.$bridge.callHandler('redScanCodeCall',msg,(res)=>{
      ...
      getpickdata: function (deliveryGoodsCode, refresh) {//是否需要刷新
        ...
        // var channelCode="JK"
        if (deliveryGoodsCode == undefined) {
          deliveryGoodsCode = ""
        }
        layer.load(2);
        ...
        // var url ="http://10.82.26.249:8888/onstore/admin/order/toTake/getTotakeOrder"//源地址
        var url = store.state.runurl + "/onstore/admin/order/toTake/getTotakeOrder"
        ...
        $.ajax({
          ...
          url: url,
          datatype: "json",
          data: { storeNo: store.state.storeNum, deliveryGoodsCode: this.dn },
          ...
          if (res.code == 0 && res.data != null && res.data != "") {
            var d = res.data.substring(0, res.data.length)
...
<style scoped>
  ...
  .searchButton {
    ...
    line-height: 6rem;
    border-radius: 3rem;
    box-shadow: 0 0.1rem 0.4rem rgba(0, 158, 150, 0.9);
  }
